...
class _EventDetailScreenState extends State<EventDetailScreen> {
  ...
  void initState() {
    ...
    _recurringDays = Set.from(e.recurringDays);
  }
  ...
  Future<void> _save() async {
    ...
    final idx = all.indexWhere(
      (e) => e.name == widget.event.name && e.dateTime == widget.event.dateTime,
    );
    if (idx != -1) {
      all[idx] = updated;
    } else {
  ...
  Future<void> _delete() async {
    ...
    await NotificationService.cancelForEvent(widget.event);
    final all = await StorageService.loadEvents();
    all.removeWhere(
      (e) => e.name == widget.event.name && e.dateTime == widget.event.dateTime,
    );
